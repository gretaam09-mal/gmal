...
- **Model call settings:** structured output via Anthropic tool use — the
  ...
  `ComposedDiffNote`'s own schema, so the API guarantees a well-formed
...
Respond by calling record_diff_note with:
...
4. Write for a PE deal team reading a diff strip: concise, factual, no
   hedging language.
5. Respond only by calling record_diff_note — no other text.
```
