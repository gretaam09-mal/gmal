...
from services.memo import create_memo_from_analysis, submit_for_review
...
def _seeded_memo(client_as, make_user, db_session, *, title):
    owner = make_user()
    ...
    db_session.commit()
    return client, workspace, owner, memo


def test_list_review_queue_orders_worst_confidence_first(client_as, make_user, db_session):
    client, workspace, _owner, memo = _seeded_memo(
    ...
    # force a specific confidence grade to make the ordering assertion exact
    version.confidence_grade = "C"
    submit_for_review(version)
    db_session.commit()

    queue = list_review_queue(db_session, uuid.UUID(workspace["id"]))
    assert len(queue) == 1
    assert queue[0].memo_id == memo.id
    assert queue[0].status.value == "in_review"
    assert queue[0].confidence_grade == "C"


def test_review_queue_excludes_approved_memos(client_as, make_user, db_session):
    client, workspace, _owner, memo = _seeded_memo(
        client_as, make_user, db_session, title="project-condor"
    )
    version = db_session.query(MemoVersion).filter(MemoVersion.memo_id == memo.id).one()

    queue_before = list_review_queue(db_session, uuid.UUID(workspace["id"]))
    assert len(queue_before) == 1

    submit_for_review(version)
    db_session.commit()

    resp = client.get(f"/workspaces/{workspace['id']}/memos/review-queue")
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert len(body) == 1
    assert body[0]["memo_id"] == str(memo.id)
    assert body[0]["status"] == "in_review"


def test_review_queue_route_is_registered_ahead_of_memo_id_route(client_as, make_user, db_session):
    """The GET .../memos/review-queue route must not be swallowed by
    GET .../memos/{memo_id} — a 422 here would mean "review-queue" got
    parsed as a memo_id path param instead."""
    owner = make_user()
    client = client_as(owner)
    workspace = _create_tenant_and_workspace(client, codename="empty-ws")
    resp = client.get(f"/workspaces/{workspace['id']}/memos/review-queue")
    assert resp.status_code == 200, resp.text
    assert resp.json() == []
